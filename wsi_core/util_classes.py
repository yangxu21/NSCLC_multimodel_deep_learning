...
class Mosaic_Canvas(object):
    ...
    def paste_patch(self, patch):
        ...
        downscaled_patch = patch.resize((self.downscaled_patch_size, self.downscaled_patch_size))
        self.canvas.paste(downscaled_patch, tuple(self.coord))
        self.increment_coord()

class Contour_Checking_fn(object):
    # Base class for contour checking
...
class isInContourV2(Contour_Checking_fn):
    ...
    def __call__(self, pt): 
        return 1 if cv2.pointPolygonTest(self.cont, tuple(np.array(pt).astype(float) + self.patch_size/2), False) >= 0 else 0
