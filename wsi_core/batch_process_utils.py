...
def initialize_df(slides, seg_params, filter_params, vis_params, patch_params, 
    ...
    default_df_dict.update({
        ...
        'keep_ids': np.full((total), str(seg_params['keep_ids'])),
        'exclude_ids': np.full((total), str(seg_params['exclude_ids'])),
    ...
    if isinstance(slides, pd.DataFrame):
        ...
        # Update with existing params from input df
        for key in slides.columns: 
            if key in temp_copy.columns:
                temp_copy[key] = slides[key].values
        return temp_copy
    else:
